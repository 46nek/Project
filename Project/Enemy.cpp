...
namespace {
	...
	constexpr float SEARCH_DURATION = 3.0f;    // 捜索時間
...
void Enemy::Update(float deltaTime, const Player* player, const Stage* stage, const std::vector<Decoy*>& decoys, const std::vector<std::unique_ptr<Enemy>>& enemies) {
	...
	if (chasingDecoy) {
		...
		case CHASE:
			if (canSee || canHear) {
				...
				targetPos = m_lastKnownPlayerPos; // 最後に一目見た場所へ行く
			}
			break;
		...
		case SEARCH:
			if (canSee || canHear) {
				...
				targetPos = m_lastKnownPlayerPos; // 捜索中は最後の位置へ向かう。着いたらウロウロさせたいが今回は位置継続
			}
			break;
	...
	// MOVE LOGIC (A*)
	// ターゲットが変わらない限り、パス再計算のクールダウンごとに行う
		...
		if (m_pathIndex != -1 && m_pathIndex < static_cast<int>(m_path.size())) {
			...
			if (distance < 0.1f) {
				...
				for (const auto& otherEnemy : enemies) {
					...
					if (distToNeighbor < 2.0f && distToNeighbor > 0.001f) { // 2.0f以内で敵から離れる
				...
				if (neighborCount > 0) {
					...
					// 同じ場所に固まり続けるのを防ぐため、混雑時は確率でターゲット変更（巡回時に限る）
