#include "Source/03_DirectX/ShaderManager.h"
...
bool ShaderManager::Initialize(ID3D11Device* device) {
	...
	if (FAILED(hr)) { OutputShaderError(errorBlob, "Shaders/VertexShader.hlsl"); return false; }
	...
	// ポストプロセス用頂点シェーダー
	hr = D3DCompileFromFile(L"Shaders/PostProcessVertexShader.hlsl", nullptr, nullptr, "VS", "vs_5_0", 0, 0, &postProcessVsBlob, &errorBlob);
	if (FAILED(hr)) { OutputShaderError(errorBlob, "Shaders/PostProcessVertexShader.hlsl"); return false; }
	...
	if (FAILED(hr)) { OutputShaderError(errorBlob, "Shaders/PixelShader.hlsl"); return false; }
	hr = device->CreatePixelShader(psBlob->GetBufferPointer(), psBlob->GetBufferSize(), nullptr, &m_pixelShader);
	if (FAILED(hr)) { psBlob->Release(); return false; }

	// テクスチャ用ピクセルシェーダー
	hr = D3DCompileFromFile(L"Shaders/TexturePixelShader.hlsl", nullptr, nullptr, "PS", "ps_5_0", 0, 0, &texturePsBlob, &errorBlob);
	if (FAILED(hr)) { OutputShaderError(errorBlob, "Shaders/TexturePixelShader.hlsl"); return false; }
	...
	// モーションブラー用ピクセルシェーダー
	hr = D3DCompileFromFile(L"Shaders/MotionBlur.hlsl", nullptr, nullptr, "main", "ps_5_0", 0, 0, &motionBlurPsBlob, &errorBlob);
	if (FAILED(hr)) { OutputShaderError(errorBlob, "Shaders/MotionBlur.hlsl"); return false; }
	...
	if (FAILED(hr)) { OutputShaderError(errorBlob, "Shaders/DepthVertexShader.hlsl"); return false; }
	...
	// UI用頂点シェーダー
	hr = D3DCompileFromFile(L"Shaders/SimpleVertexShader.hlsl", nullptr, nullptr, "main", "vs_5_0", 0, 0, &simpleVsBlob, &errorBlob);
	if (FAILED(hr)) { OutputShaderError(errorBlob, "Shaders/SimpleVertexShader.hlsl"); return false; }
	...
	// UI用ピクセルシェーダー
	hr = D3DCompileFromFile(L"Shaders/SimplePixelShader.hlsl", nullptr, nullptr, "main", "ps_5_0", 0, 0, &simplePsBlob, &errorBlob);
	if (FAILED(hr)) { OutputShaderError(errorBlob, "Shaders/SimplePixelShader.hlsl"); return false; }
