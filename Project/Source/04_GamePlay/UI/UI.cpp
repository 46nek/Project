...
#include "Source/00_Core/Game.h"
#include "Source/06_Other/AssetPaths.h"
#include <string>
#include <cstdio>
#include <algorithm>
...
void UI::RenderPauseMenu(int selectIndex, int screenWidth, int screenHeight) {
	...
	if (m_fontWrapper) {
		...
		float menuTop = screenHeight * 0.28f;
		float menuLeft = screenWidth * 0.05f;
		const wchar_t* items[] = { L"SETTINGS", L"CONTROLS" };
		...
		UINT32 retColor = (m_hoverIndex == 2 || selectIndex == 2) ? 0xFF00FFFF : 0xFFFFFFFF;
		...
		UINT32 exitColor = (m_hoverIndex == 3 || selectIndex == 3) ? 0xFF00FFFF : 0xFFFFFFFF;
