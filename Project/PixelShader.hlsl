...
float4 PS(VS_OUTPUT input) : SV_Target
{
    ...
    {
        // --- ▼▼▼ 修正点 2/2 ▼▼▼ ---
        // わずかなバイアスを追加して、シャドウアクネ（自分自身の影）を防ぐ
        float depthValue = input.LightViewPos.z / input.LightViewPos.w;
        shadowFactor = shadowMapTexture.SampleCmpLevelZero(ShadowSampleType, projectTexCoord, depthValue - 0.001f);
    }
