...
float4 HorizontalBlurPS(float4 pos : SV_POSITION, float2 tex : TEXCOORD) : SV_Target
{
    ...
    {
        color += shaderTexture.Sample(Sampler, tex + float2(texelSize.x * i, 0.0f)) * WEIGHTS[i];
        color += shaderTexture.Sample(Sampler, tex - float2(texelSize.x * i, 0.0f)) * WEIGHTS[i];
    }
